...
**What `null` actually means — read this before writing one.** `null` selects the **device** clock, not "the browser". A client component's *first* render happens on the server, where the process clock is UTC, so `null` produces a UTC day in the SSR payload and the device's day only after hydration. `null` is therefore correct only where a post-hydration flip is acceptable: transient display text, a default form value, a chip label. It is **wrong** wherever the value is compared against something computed server-side in the stored zone — a `dateKey` from `freeGaps`, an `occurred_at` stamped by `todayKey`, a projection anchored to the page's `today` — because for a user whose device zone differs from `user_settings.timezone` that disagreement is permanent, not a flash. In those cases thread the server's day down as a prop — `FinanceCalendar`/`FinanceClient` and `StreamClient`/`CardRow` all take a required `today` sourced from the page's own resolution.

**The test to apply, because "it's a client component" is not one.** Ask whether the value is *rendered* or *compared/written*. Comparing it against anything the server produced in the stored zone — a `freeGaps` `dateKey`, a `todayKey`-stamped `occurred_at`, the NOW/NEXT/LATER sectioning `streamSnapshot` already computed — means the two clocks must agree, so thread the server's day. Writing it is stricter still: a day that reaches a date column (`createTask`'s `dueDate`, a snooze key passed to `updateTask`) is **never** display-only, because a wrong value persists rather than healing on hydration. The stream is the worked example: all four of its `todayISO(null)` sites initially looked like labels, and every one turned out to gate a destructive action, seed a written date key, or contradict the server's own classification. Remaining `null` sites are genuinely render-only: `tasks-client.tsx`, `task-row.tsx`, `dock.tsx`, `inventory-client.tsx`, and the finance form defaults.
