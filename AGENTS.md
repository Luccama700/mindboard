...
**The invariant has a regression net.** `test/isolation-proof.mjs` (+ `test/isolation/*`) creates two throwaway tenants, seeds identifiable rows in every domain, and calls **every registered tool** with tenant A's token but tenant B's object ids *and names* — asserting both that no B data comes back and that no B row moved (a full before/after row snapshot of B). Coverage is checked against the live `tools/list`, so **registering a tool without adding an attack-table entry in `test/isolation/tool-matrix.mjs` fails the probe** instead of silently shrinking coverage. It also drives the OAuth authorization server over real HTTP (consent binding, redirect equality, PKCE, replay, wrong client, cross-site GET, registration host allowlist). Run it against a **local** dev server: `npm run dev` then `MCP_URL=http://localhost:3000/api/mcp/mcp node test/isolation-proof.mjs`. It needs `MCP_OAUTH_SECRET` set for the OAuth section and deletes both tenants in a `finally`. The probe's own detectors (leak scanner, snapshot diff, coverage report) are unit-tested in `__tests__/isolation-probe.test.ts` so they cannot rot into always-passing.
