...
**Why this bites by default, not just while travelling.** `user_settings.timezone` is `null` until the user visits `/settings` and saves — `preferences-form.tsx` only *guesses* it as a form default. For an unconfigured account the server's day is the UTC process clock while the device's is local, so the two diverge **every evening after 17:00 PDT**. Populating that column on first load would remove the whole failure mode; until then, treat "server day ≠ device day" as the normal case.

Only four `todayISO(null)` calls remain, all verified render-only: `inventory-client.tsx` (run-out hints, shopping-list preview, the "sitting at zero" nudge) and `inventory-calendar.tsx` (depletion projection). None is compared against a server key or written anywhere.
