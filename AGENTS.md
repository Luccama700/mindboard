...
- **Strong**: propose-time resolution refuses B's ids and names on every write tool (including `update_stock`'s fuzzy exact/substring name matching, and `manage_finance`/`update_finance` name refs); the executors refuse B's ids when reached through a forged proposal (with a control forgery on A's own row proving the executors are genuinely reached); list/filter reads return only A's rows; OAuth consent binding, redirect equality and PKCE hold over real HTTP.
