...
**The invariant has a partial regression net.** `test/isolation-proof.mjs` (+ `test/isolation/*`) creates two throwaway tenants, seeds identifiable rows in every domain, and calls **every registered tool** with tenant A's token but tenant B's object ids *and names*. Coverage is checked against the live `tools/list`, so **registering a tool without adding an attack-table entry in `test/isolation/tool-matrix.mjs` fails the probe** instead of silently shrinking coverage. It also drives the OAuth authorization server over real HTTP (consent binding, redirect equality, PKCE, replay, wrong client, cross-site GET, registration host allowlist), and forges `ai_audit_log` rows to reach `confirm_action`'s executors across the boundary — which a cross-tenant propose can never do, since propose-time resolution refuses the id first.

Read the pass count as *"these specific claims held"*, not *"isolation is proven"*. What it actually establishes:

- **Strong**: propose-time resolution refuses B's ids and names on every write tool (including `update_stock`'s fuzzy exact/substring name matching, and `manage_finance`/`update_finance` name refs); eleven executors refuse B's ids when reached through a forged `ai_audit_log` row, **each paired with its own control** — the identical forged shape aimed at a scratch row of A's, which must APPLY, so a resolved-op shape that has drifted from the executor's validator turns red and names the executor instead of making the refusal vacuous (`test/isolation/forged-proposal.mjs`; adding an entry without a control fails `__tests__/isolation-probe.test.ts`); list/filter reads return only A's rows; OAuth consent binding, redirect equality and PKCE hold over real HTTP.
- **Weak by construction — a scoping breach in these would NOT turn the probe red.** Ten tools pass because a precondition is missing rather than because isolation holds: `list_events`, `schedule_snapshot`, `reschedule_event` and `create_event` (no `google_tokens` row — `getScheduleSnapshot` calls `listEvents` unconditionally and throws), `list_brain_notes`, `read_brain_note` and `capture_to_brain` (no `vault_settings` row), `lookup_prices` (no Anthropic key), `process_reel` (tenant not worker-allowlisted), `list_code_tasks` (no group named "mindboard"). They are marked `weak` in the coverage table with the reason; making them real needs seeded per-tenant credentials. `finance_forecast` returns only derived numbers, so it is asserted on *values* (net worth ≠ A+B), and preference scoping is asserted through `get_snapshot`'s wake-window-derived free gaps — which is why the two tenants are seeded with **different** currencies, timezones and wake windows. Do not "tidy" those to match.
- **The row snapshot is a hand-maintained table list** (`OWNED_TABLES` in `test/isolation/seed.mjs`). A user-scoped table missing from it is invisible to the "nothing of B's changed" check. Add every new user-scoped table there; the probe fails if a listed table stops being readable, but it cannot detect one that was never listed.

Run it against a **local** dev server: `npm run dev` then `MCP_URL=http://localhost:3000/api/mcp/mcp node test/isolation-proof.mjs`, with `MCP_OAUTH_SECRET` set for the OAuth section. **A local dev server is not a local database** — the probe writes to whatever `.env.local` points at, which on a normal checkout is the live project. It creates two real auth users, deletes them in a `finally` (registered before anything can throw) and on SIGINT/SIGTERM, and verifies the deletion. The probe's own detectors (leak scanner, snapshot diff, coverage report) are unit-tested in `__tests__/isolation-probe.test.ts` so they cannot rot into always-passing.
...
- **Home worker** (`worker/worker.py` + `worker/README.md`): pull-based, no inbound ports, no DB key on the PC — it polls `POST /api/worker` (bearer = `MCP_BEARER_TOKEN`) for claim/heartbeat/complete/fail; claims are atomic (`claim_next_job(allowed_users)`, stale-heartbeat reclaim, 3-attempt dead-letter) and **allowlist-gated**: only jobs from the owner + `WORKER_ALLOWED_USER_IDS` (comma-separated ids; `workerAllowedUserIds()` in `app/lib/mcp/config.ts`) are claimed, and the enqueue paths (`queueSourceConversion`, the vibevoice branch of `generateEpisodeFor`) refuse non-allowlisted users up front. The post-claim ops re-assert the same allowlist rather than trusting the job id: `loadJob` (used by complete/fail and the mid-finalize retry) and the heartbeat update both filter `user_id` through `workerAllowedUserIds()`, so a bearer holder cannot drive another tenant's job by guessing its id. PDFs travel via signed download URLs and audio via signed upload URLs; all vault/Postgres finalization stays in the app. The settings connections card shows online/queue state from `worker_status`.
