...
`0053_task_energy_subtasks.sql` adds `tasks.energy_cost` (smallint 1-5), `tasks.energy_source` (`'ai'`/`'user'`), `tasks.parent_task_id` (self-FK, on delete cascade, depth one enforced in the app) and `tasks.not_before` (date, CHECK `<= due_date`). See the Energy & decomposition section.
...
- Energy: a five-dot control (`app/_components/energy-dots.tsx`); one tap sets `energy_cost` with `energy_source='user'`, tapping the current level clears it. Outlined dots = the AI default, filled = the user's.
