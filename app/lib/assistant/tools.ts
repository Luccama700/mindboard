...
export const ASSISTANT_TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    name: "list_people",
    description:
      "The people roster: id, name, vault note path, aliases, their context group ({id,name} or null — groups are contexts like family/ubc/work, never closeness tiers), check-in cadence, archived flag, how many interactions are logged, and when the user last TALKED to them (date + precision + daysSince, or null when nothing is logged). Metadata only — no note bodies, no interaction summaries. Use it to find person ids/names before propose_update_people or get_person; pass includeArchived to also see untracked people (needed before a restore op). 'Last talked' counts logged conversations ONLY: editing someone's note is being informed, not being in touch, and never advances it.",
    input_schema: {
      type: "object",
      properties: { includeArchived: { type: "boolean" } },
      additionalProperties: false,
          ...
          items: {
            ...
            properties: {
              ...
              name: {
                type: "string",
                description:
                  "New person's or group's name (create_person, create_group).",
              },
              group: {
                type: ["string", "null"],
                description:
