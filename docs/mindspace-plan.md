...
- **M1 — foundation** *(shipped 2026-07-22)*: migration 0043 (`mindspace_topics`
  only — fast-path classifications are deterministic and recomputed at read time,
  so the item/label/observation tables wait for M2, when LLM results stop being
  derivable), taxonomy seeding flow, fast-path classifier
