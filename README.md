...
- `POST /api/watch/followup` `{ "id", "text" }` — queue a dictated follow-up on a task as a `followup` job for the home worker (`worker/worker.py`), where Claude Code turns it into a new task in the same group with the same due date; the original stays open. `meta.followups` in `/today` reports pending and recently failed jobs.
