...
- `GET /api/watch/today` — overdue + due-today tasks (with group, priority, and notes for the detail screen), today's remaining calendar events, today's routines with done state, upcoming events and tasks for the next 7 days, next timed event, free hours left, plus `meta` (server time, user timezone).
